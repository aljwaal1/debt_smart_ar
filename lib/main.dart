...
class Entry {
  ...
  static Entry decode(String raw) { final p = raw.split('|||'); return Entry(p.isNotEmpty ? p[0] : 'اسم', p.length > 1 ? double.tryParse(p[1]) ?? 0 : 0, p.length > 2 ? p[2] == '1' : true, p.length > 3 ? p[3] : '', p.length > 4 ? DateTime.tryParse(p[4]) ?? DateTime.now() : DateTime.now()); }
}
...
class _HomeState extends State<Home> {
  ...
  Future<void> load() async { final p = await SharedPreferences.getInstance(); final data = p.getStringList('ledger_v4') ?? p.getStringList('ledger_v3') ?? []; setState(() => rows = data.map(Entry.decode).toList()); }
  Future<void> save() async { final p = await SharedPreferences.getInstance(); await p.setStringList('ledger_v4', rows.map((e) => e.encode()).toList()); }
  ...
  List<Entry> get visible { final q = search.text.trim(); if (q.isEmpty) return rows; return rows.where((e) => e.name.contains(q) || e.note.contains(q)).toList(); }

  void add() { final n = name.text.trim(); final v = double.tryParse(amount.text.trim()) ?? 0; if (n.isEmpty || v <= 0) return; setState(() { rows.insert(0, Entry(n, v, incoming, note.text.trim(), DateTime.now())); name.clear(); amount.clear(); note.clear(); }); save(); SystemSound.play(SystemSoundType.click); }
  void removeEntry(Entry e) { final i = rows.indexOf(e); setState(() => rows.remove(e)); save(); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('تم الحذف'), action: SnackBarAction(label: 'تراجع', onPressed: () { setState(() => rows.insert(i < 0 ? 0 : i, e)); save(); }))); }
  void copyReport() { final text = StringBuffer()..writeln('$appTitle $appVersion')..writeln('الإجمالي الأول: ${totalIn.toStringAsFixed(2)}')..writeln('الإجمالي الثاني: ${totalOut.toStringAsFixed(2)}')..writeln('الصافي: ${net.toStringAsFixed(2)}')..writeln('---'); for (final e in rows) { text.writeln('${e.name} | ${e.incoming ? 'إضافة' : 'خصم'} | ${e.amount.toStringAsFixed(2)} | ${e.note}'); } Clipboard.setData(ClipboardData(text: text.toString())); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم نسخ الكشف'))); }

  @override Widget build(BuildContext context) { final pages = [dashboard(), formPage(), report(), about()]; return Scaffold(body: SafeArea(child: AnimatedSwitcher(duration: const Duration(milliseconds: 220), child: pages[tab])), bottomNavigationBar: NavigationBar(selectedIndex: tab, onDestinationSelected: (v) => setState(() => tab = v), destinations: const [NavigationDestination(icon: Icon(Icons.home_rounded), label: 'الرئيسية'), NavigationDestination(icon: Icon(Icons.add_rounded), label: 'إضافة'), NavigationDestination(icon: Icon(Icons.receipt_long_rounded), label: 'السجل'), NavigationDestination(icon: Icon(Icons.info_rounded), label: 'عن')])); }
